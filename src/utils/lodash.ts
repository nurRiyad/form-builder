...
function getValue(obj: any, pathArr: Array<string>): unknown {
    ...
    return isArrayKey
        ? getValue(value[index], pathArr.slice(1))
        : getValue(value, pathArr.slice(1));
}
...
function setValue(obj: Record<string, unknown>, pathArr: Array<string>, valueToSet: unknown): void {
    ...
    if (isArrayKey) {
        ...
        setValue((obj[key] as Array<unknown>)[index] as Record<string, unknown>, pathArr.slice(1), valueToSet);
    } else {
